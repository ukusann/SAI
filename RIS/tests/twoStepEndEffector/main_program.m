...
armJoints(4) = 0*pi/180; % theta4 needs to be positive always
...
while stop==0
    ...
    if(dir_flag == 0)
        ...
        conveyor_dist = 0.22;
        if(mushroomCanPos(1,1) > -0.79 && twoStep == 0)
            desPosition = mushroomCanPos(1, 1:3);
            desPosition(1, 1) = desPosition(1, 1) + conveyor_dist;
        ...
        elseif(twoStep == 1)
            ...
            elseif(start_traj == 1)
                ...
                for i = 1:size(lastSolution, 1)
                    joints_traj(i, 1) = lastSolution(i, 1) + delay_dir^2*(optimalSolution(1, i) - lastSolution(i))/3 - delay_dir^3*(optimalSolution(1, i) - lastSolution(i))*2/27
                end
    
                robot_arm.set_joints(joints_traj);
                if(delay_dir > 3)
                    delay_dir = 0;
                    dir_flag = 1;
                    twoStep = 0;
    ...
    elseif(dir_flag == 2)
        ...
        if(delay_dir > 3)
            ...
            desPosition(1, 2) = desPosition(1, 2) - 0.15;
            desPosition(1, 3) = desPosition(1, 3) + 0.15;
            poseHand = refWorldToBase(desPosition', rpy_des_deg2')
            [error, solutionsNum, joingAnglesSol1, joingAnglesSol2, joingAnglesSol3, joingAnglesSol4] = kuka_kinematics.inverseKinematics(alpha, poseHand);
            if(error == 1)
                sim.terminate();
                return;
