...
while itarget<=sim.TARGET_Number % until robot goes to last target (TARGET_Number)
    ...
    %% FSM
    if(tempFlag == 1)
        fsm = py.fsm.StateMachine;
        disp(['Current State', char(fsm.state)]); % Print using matlab

        fsm.transition('event1')
        py.print("Current State:", fsm.state) % Print using python

        fsm.transition('event2')
        py.print("Current State:", fsm.state)
