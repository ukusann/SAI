...
%***** Parking System *****
phi_parking = [pi/2, pi, 0, pi/2, pi, pi, 0];
delay_exitPark = 0;
delay_rotate = 0;
%***********************
        ...
        if(waitToMove == 1)
            ...
            if(itarget == 4)
                ...
                elseif(itarget == 6)
                    ...
                    targetPose = [0.1, -2.475, 0.650, 0, 0, 90]; % Pose ShelfLeft
                    [delay_movArm, setJoints, calcInvKin, joints, poseHand, stopTraj] = stateMachine.handlerMoveArmEndEffector(lastSolution, finalSolution, start_traj, ...
                        delay_movArm, start, targetPose, robotPoseInWorldRef, box_high,itarget);
                elseif(itarget == 7)    
                    ...
                    [delay_movArm, setJoints, calcInvKin, joints, poseHand, stopTraj] = stateMachine.handlerMoveArmEndEffector(lastSolution, finalSolution, start_traj, ...
                                                                                                                    delay_movArm, start, boxesPose(4, :), robotPoseInWorldRef, box_high,itarget);
            end
    ...
    if(currentState == states.PickBox)
        ...
        if(waitToMove == 1)
            ...
            [delay_grip, setJoints, calcInvKin, joints, poseHand, closeGripper] = stateMachine.handlerPickBox(lastSolution, finalSolution, start_traj, ...
                                                                                                                delay_grip, start, boxesPose(4, :), robotPoseInWorldRef, box_high, itarget);
            if(setJoints == 1)
                setJoints = 0;
                error = robot_arm.set_joints(joints);
                if(error == 1)
