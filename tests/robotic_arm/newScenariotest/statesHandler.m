...
classdef statesHandler < handle
    ...
    methods
        ...
        function [delay_out, setJoints, calcInvKin, joints_out, desPoseHand, stopTraj] = handlerMoveArmEndEffector(obj, initJoints, finalJoints, startTraj, delay, start, poseObj, poseRobot, itarget)
            delay_out = 0;
            setJoints = 0;
            calcInvKin = 0;
            ...
            if(startTraj == 1)
                ...
                if(itarget == 4)
                    ...
                    % rot07 = [-rotObj(1:3, 2), -rotObj(1:3, 1), -rotObj(1:3, 3)];0
                    [des_yaw_x, des_pitch_y, des_roll_z] = kinematics.computeMatrixToRPY(rot07);
                    desPoseHand = [poseObjInBaseRef(1), poseObjInBaseRef(2), poseObjInBaseRef(3)+obj.distanceHand+0.05, des_yaw_x, des_pitch_y, des_roll_z]';
                else
                    rot07 = [rotObj(1:3, 1), -rotObj(1:3, 2), -rotObj(1:3, 3)];
                    [des_yaw_x, des_pitch_y, des_roll_z] = kinematics.computeMatrixToRPY(rot07);
        ...
        function [delay_out, setJoints, calcInvKin, joints_out, desPoseHand, closeGripper] = handlerPickBox(obj, initJoints, finalJoints, startTraj, delay, start, poseObj, poseRobot, box_high)
            delay_out = 0;
            setJoints = 0;
            calcInvKin = 0;
            desPoseHand = zeros(1, 6);
            ...
            if(startTraj == 1)
                ...
                [des_yaw_x, des_pitch_y, des_roll_z] = kinematics.computeMatrixToRPY(rot07);
                if(box_high == 1)
                    desPoseHand = [poseObjInBaseRef(1)-0.2, poseObjInBaseRef(2), poseObjInBaseRef(3)+obj.distanceHand-0.05, des_yaw_x, des_pitch_y, des_roll_z]';
                else
                    desPoseHand = [poseObjInBaseRef(1), poseObjInBaseRef(2), poseObjInBaseRef(3)+obj.distanceHand, des_yaw_x, des_pitch_y, des_roll_z]';
                end
                calcInvKin = 1;
            end
        end
        ...
        function [delay_out, setJoints, calcInvKin, joints_out, desPoseHand, openGripper] = handlerPlaceBox(obj, initJoints, finalJoints, startTraj, delay, start, poseObj, poseRobot, box_high, itarget)
            delay_out = 0;
            setJoints = 0;
            calcInvKin = 0;
            desPoseHand = zeros(1, 6);
