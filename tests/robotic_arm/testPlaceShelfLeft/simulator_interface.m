...
%   2022/03/17
    ...
    methods
        ...
        function [error, convOutPose] = get_conveyorOutPose(obj)
            %UNTITLED2 Summary of this function goes here
            %   Detailed explanation goes here
            % Pose of boxes
            error = 0;
            [res,convOutPos] = obj.vrep.simxGetObjectPosition(obj.clientID,obj.ConveyorOutHandle,-1,obj.vrep.simx_opmode_blocking);
            if (res ~= obj.vrep.simx_return_ok && res ~= obj.vrep.simx_return_novalue_flag)
                disp('ERROR: Failed getting conveyorOut position!');
                error = 1;
                return;
            end
            
            [res,convOutOrientation] = obj.vrep.simxGetObjectOrientation(obj.clientID,obj.ConveyorOutHandle,-1,obj.vrep.simx_opmode_blocking);
            if (res ~= obj.vrep.simx_return_ok && res ~= obj.vrep.simx_return_novalue_flag)
                disp('ERROR: Failed getting conveyorOut orientation!');
                error = 1;
                return;
            end

            convOutPose = [convOutPos, convOutOrientation];
